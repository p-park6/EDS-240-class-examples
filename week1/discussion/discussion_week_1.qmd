...
fracking_clean <- fracking |> 
  ...
  # clean column names ----
  janitor::clean_names() |> #converted column names to lowercase and included underscore between words. Better readability of column headers.
  
  # clean up dates ----
  mutate(job_start_date = str_remove(job_start_date, " AM")) |> # remove 'AM' from string from the job_start_date column cells. dis: remove the string "AM" from the job_start_date column cells.
  mutate(datetime_start = mdy_hms(job_start_date)) |> # convert from string to date (save to new col). dis: create a new column called "datetime_start
